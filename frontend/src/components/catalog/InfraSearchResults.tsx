...
import { slugify, type Pic, type Server } from "@/lib/types";
import { InfraStatusBadge } from "./InfraBits";
import { Card } from "@/components/ui/card";

/**
 * Infrastructure results for the catalog search box: matching servers and PICs
 * alongside the application results. Both open their detail drawers.
 */
export function InfraSearchResults({ search }: { search: string }) {
  ...
  const picHits = pics ?? [];
  if (!enabled || (serverHits.length === 0 && picHits.length === 0)) return null;

  return (
    <Card className="mb-4 p-4" data-testid="infra-search-results">
      <h2 className="text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
        Infrastructure matches for “{query}”
      </h2>
